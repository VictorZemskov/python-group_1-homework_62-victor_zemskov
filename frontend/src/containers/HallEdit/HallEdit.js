...
class HallEdit extends Component {
    state = {
        // исходные данные зала, загруженные из API.
        hall: null,
        ...
        axios.get(HALL_URL + this.props.match.params.id)
            .then(response => {
                ...
                this.setState(prevState => {
                    ...
                    newState.hall = hall;
                    // newState.movie.category_name = movie.category_name.map(category => category.id);
                    return newState;
                });
            })
            .catch(error => {
    ...
    formSubmitted = (hall) => {
        ...
        return axios.put(HALL_URL + this.props.match.params.id + '/', formData, {
            ...
            .then(response => {
                ...
                const hall = response.data;
                console.log(hall);
                // если всё успешно, переходим на просмотр страницы зала с id,
                // указанным в ответе
